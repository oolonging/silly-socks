...
namespace UI_Elements {
	...
	void DialogueBox::draw(void) {
		...
		if (speakerName != nullptr) {
			...
			Text::text(speakerName, boxX + textPadding, boxY - textPadding);
		}

		// Calculate sprite size based on dialogue box height
		float availableHeight = height - nameBoxHeight - (textPadding * 3);
		float calculatedSpriteSize = min(spriteSize, availableHeight);
		...
		if (showSprite && characterSprite != nullptr) {
			...
			Graphics::image(spriteX, spriteY, calculatedSpriteSize, calculatedSpriteSize, characterSprite, Shapes::CORNER);
			AEGfxSetRenderMode(AE_GFX_RM_COLOR);

			// Adjust text start position to account for sprite
			textStartX = spriteX + calculatedSpriteSize + textPadding;
		}
